...
+(instancetype)cellWithTableView:(UITableView *)tableView{
    ...
    SCDiscountCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
    if (cell == nil) {
        cell = [[SCDiscountCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifier];
    }
...
-(void)setDiscountArray:(NSMutableArray *)discountArray{
    ...
    for (int j = 0; j < 4; ++j) {
        ...
        SCDiscountModel *discount = discountArray[j];
...
-(void)OnTapBackView:(UITapGestureRecognizer *)sender{
    ...
    SCDiscountModel *discountModel = _discountArray[index];
