...
@end
@implementation SCKindFilterCell
...
+(instancetype)cellWithTableView:(UITableView *)tableView {
    //  这个 静态字符串不要与类名相同
    static NSString *KindFilterCell = @"SCKindFilterCell";
    SCKindFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:KindFilterCell];
    if (cell == nil) {
        //注：这个地方要留一个心，看是调父类的还是构造方法的
        cell = [[SCKindFilterCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:KindFilterCell];
...
-(void)setMerchantCataGroupModel:(SCMerchantCataGroupModel *)merchantCataGroupModel{
