//
//  SCSearchHistoryViewController.m
//  meituan
//
...
@interface SCSearchHistoryViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
@property(nonatomic ,strong)UITableView *searchTableView;

/** 搜索文本框 */
@property (nonatomic, weak) SCSearchTextField *searchTF;
/** 搜索的tableView */
...
@implementation SCSearchHistoryViewController
...
-(void)initNav{
    ...
    SCSearchTextField *search = [[SCSearchTextField alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.datas.count == 2 && indexPath.section == 0 && self.hisDatas.count) {
        SCSearchHistoryCell *cell = [SCSearchHistoryCell cellWithTableView:tableView IndexPath:indexPath atNSMutableArr:self.hisDatas];
        cell.searchHistoryLabel.text = self.datas[0][indexPath.row];
        return cell;
    } else {
        //这里就一个cell 不用注册了
        SCSearchHotCellCell *cell = [SCSearchHotCellCell cellWithTableView:tableView ];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    SCSearchHeaderView *headerView = [SCSearchHeaderView headViewWithTableView:tableView];
    if (self.hisDatas.count) {
        if (section == 0) {
