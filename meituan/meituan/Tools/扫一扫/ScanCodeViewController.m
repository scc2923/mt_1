...
//                    [[[UIAlertView alloc] initWithTitle:nil message:@"本应用无访问相机的权限，如需访问，可在设置中修改" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil, nil] show];
                    ...
                    //设置扫描区域，这个需要仔细调整
                    [output setRectOfInterest:CGRectMake(64/SCREENHEIGHT, (SCREENWIDTH - 320)/2/SCREENWIDTH, 320/SCREENHEIGHT, 320/SCREENWIDTH)];
...
- (void)_initView {
    ...
    //扫码框
    _backImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 64, s, SCREENHEIGHT - 64)];
    _backImageView.image = [UIImage imageNamed:@"camera_bg"];
    [self.view addSubview:_backImageView];

    
    _lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake(16, 15, SCREENWIDTH - 32, 1)];
...
-(void)lineAnimation {
    if (upOrDown == NO) {
        lineNum ++;
        _lineImageView.frame = CGRectMake(CGRectGetMinX(_lineImageView.frame), 15 + lineNum, SCREENWIDTH - 32, 1);
